...
import { TOKENS } from '../validate-ether-amount';
...
describe('validateEtherAmount', () => {
  it('should work', () => {
    validateEtherAmount(field, 12n, TOKENS.stETH);
  });

  it('should throw right error on null amount', () => {
    const fn = () => validateEtherAmount(field, undefined, TOKENS.stETH);
    expect(fn).toThrowError();
    try {
      fn();
    } catch (e) {
  ...
  it('should throw right error on zero', () => {
    const fn = () => validateEtherAmount(field, 0n, TOKENS.stETH);
    expect(fn).toThrowError();
    try {
      fn();
    } catch (e) {
  ...
  it('should throw right error on more than ethereum max uint', () => {
    const fn = () => validateEtherAmount(field, maxUint256 + 1n, TOKENS.stETH);
    expect(fn).toThrowError();
    try {
      fn();
    } catch (e) {
